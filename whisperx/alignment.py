...
def align(
    ...
    if preprocess and model_type == 'huggingface':
        print(f"Loading align model Huggingface processor for audio feature extraction (Normalization)...")
        processor = Wav2Vec2Processor.from_pretrained(
            DEFAULT_ALIGN_MODELS_HF[model_lang])
    else:
        print(f"Not loading align model Huggingface processor for audio feature extraction (Normalization)...")
